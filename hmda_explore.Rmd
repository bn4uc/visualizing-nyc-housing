...
census_tract_merged <- approval_rate_5yr_changes %>% filter(as_of_year == 2017) %>% mutate(approval_rate_change = prior - approval_rate) %>% right_join(census_tracts_bk,  by = c("census_tract_number" = "ct2010"))
...
hmda_ny_5yrs <- hmda_ny_5yrs %>% mutate(win_income_as_part_of_loan = win_income/win_loan_amount) 

hmda_ny_5yrs %>% filter(as_of_year == 2017) %>% select(win_income_as_part_of_loan, census_tract_number) %>% left_join(census_tracts, by = c("census_tract_number" = "ct2010")) %>% ggplot() + geom_sf(aes(fill = win_income_as_part_of_loan))
```
