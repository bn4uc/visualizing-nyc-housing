...
ggplot() + geom_sf(data = census_tract_merged, 
        ...
        panel.background = element_rect(fill = "#ffffff", color = NA),
        axis.line = element_blank(),
        legend.position = c(.1, .8), legend.background = element_blank(),
        text = element_text(family = "Meiryo"), 
        axis.ticks = element_blank()) + labs(title = "Approval rates for low-income applicants in Brooklyn have decreased significantly \nin gentrifying areas such as Bed-Stuy, East Williamsburg, and Crown Heights", subtitle = "However, many areas slightly further from Manhattan show increased approval rates", caption = "Home Mortgage Disclosure Act Data, CFPB, 2013-2017")

```

```{r}
#approval rates for low income applicants, faceted by year 
hmda_ny_5yrs <-  hmda_ny_5yrs %>% mutate(income_bins = ifelse(applicant_income_000s %in% c(1:73), "Income Quartile 1", 
                                             ifelse(applicant_income_000s %in% c(74:108), "Income Quartile 2", 
                                                    ifelse(applicant_income_000s %in% c(109:180), "Income Quartile 3", 
                                                           ifelse(applicant_income_000s %in% c(181:133549), "Income Quartile 4", NA)))))

apps_outcome_Q1 <- hmda_ny_5yrs %>% filter(income_bins == "Income Quartile 1") %>% group_by(census_tract_number, as_of_year, outcome) %>% summarise(outcome_tot = n())

apps_total_Q1 <- hmda_ny_5yrs %>% filter(income_bins == "Income Quartile 1") %>% group_by(census_tract_number, as_of_year) %>% summarise(tot_apps = n())

approval_rate_5yr_changes <- apps_outcome_Q1 %>% left_join(apps_total_Q1, by = c("census_tract_number", "as_of_year")) %>% filter(outcome == "Approved") %>% mutate(approval_rate = outcome_tot/tot_apps)

census_tract_merged <- approval_rate_5yr_changes %>% right_join(census_tracts,  by = c("census_tract_number" = "ct2010")) %>% filter(!is.na(as_of_year))

census_tract_merged$breaks <- cut(census_tract_merged$approval_rate, c(0, 0.2, 0.4, 0.6, 0.8, 1))


ggplot() + geom_sf(data = census_tract_merged, 
                   aes(fill = breaks), color = "white", lwd = 0) + guides(fill=guide_legend(title="Approval Rate")) +  facet_wrap(~as_of_year) + scale_fill_manual(values = c('#ffffd4', '#fdd2c0', '#f3a683', '#e27a48', '#cc4c02')) + theme(axis.text=element_blank(),                      
        axis.title=element_blank(),                      
        panel.background = element_rect(fill = "#ffffff", color = NA),
        axis.line = element_blank(),
        legend.position = c(.8, .3), legend.background = element_blank(),
        text = element_text(family = "Meiryo"), 
        axis.ticks = element_blank()) + labs(title ="Approval rates for low income applicants decline across most boroughs between 2013 and 2017", subtitle = "Staten Island does not see a general decrease in approval rates" ,caption = "Home Mortgage Disclosure Act Data, CFPB, 2013-2017")


#try with NTA areas, so that there is less breakup(though this might be confusing with the lines etc, but maybe remove the borders)

apps_outcome_Q1_nta <- hmda_ny_5yrs %>% filter(income_bins == "Income Quartile 1") %>% left_join(census_tracts, by = c("census_tract_number" = "ct2010")) %>% group_by(ntaname, as_of_year, outcome) %>% summarise(outcome_tot = n())

apps_total_Q1_nta <- hmda_ny_5yrs %>% filter(income_bins == "Income Quartile 1") %>% left_join(census_tracts, by = c("census_tract_number" = "ct2010")) %>% group_by(ntaname, as_of_year) %>% summarise(tot_apps = n())

approval_rate_5yr <- apps_outcome_Q1_nta %>% left_join(apps_total_Q1_nta, by = c("ntaname", "as_of_year")) %>% filter(outcome == "Approved") %>% mutate(approval_rate = outcome_tot/tot_apps)

#need to merge back with census hmda and then merge that with census tracts
approval_rate_5yr %>% left_join(census_tracts, by = "ntaname") %>% ggplot() + geom_sf(aes(fill = approval_rate)) + facet_wrap(~as_of_year)
...
num_apps_change_bk$change <-as.numeric(num_apps_change_bk$prior -num_apps_change_bk$n)
num_apps_change_bk$breaks <- cut(num_apps_change_bk$change, c(-7500, -500, 0, 500, 7500))

num_apps_change_bk <- num_apps_change_bk %>% left_join(census_tracts_bk, by = "ntaname")

ggplot() + geom_sf(data = num_apps_change_bk, aes(fill = breaks))+  scale_fill_manual(values = c('#cc4c02', '#e27a48', "#a6d96a", "#1a9641"))
...
ggplot(test) + 
  geom_sf(aes(fill = breaks), color = "white", lwd = 0) +  scale_fill_manual(values = c("#1a9641", "#a6d96a", '#e27a48', '#cc4c02'))
