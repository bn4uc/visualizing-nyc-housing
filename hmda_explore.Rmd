...
ggplot() + geom_sf(data = census_tract_merged, 
        ...
        text = element_text(family = "Meiryo"), 
        axis.ticks = element_blank()) + labs(title = "Approval rates for low-income applicants in Brooklyn have decreased significantly \nin gentrifying areas such as Bed-Stuy, East Williamsburg, and Crown Heights", subtitle = "However, many areas slightly further from Manhattan show increased approval rates")
...
num_apps_year$change <-as.numeric(num_apps_year$prior -num_apps_year$tot_apps)

census_tracts_bk %>% left_join(num_apps_year, by = c("ct2010" = "census_tract_number"))
...
ggplot(test) + 
  geom_sf() + geom_point(data = test2, aes(x = lon, y = lat, size = mean_loan_income_ratio))
#sort of works 
