...
approval_rate_5yr %>% left_join(nta_shape, by = "ntacode") %>% ggplot() + geom_sf(aes(fill = breaks), lwd = 0) + facet_wrap(~as_of_year) + scale_fill_manual(values = c('#ffffd4', '#fdd2c0', '#f3a683', '#e27a48', '#cc4c02')) + guides(fill=guide_legend(title="Approval Rate")) + theme(axis.text=element_blank(),                      
        ...
        text = element_text(family = "Meiryo"), 
        axis.ticks = element_blank()) + labs(title ="Approval rates for low income applicants decline in Manhattan, the Bronx, and Brooklyn between 2013 and 2017", subtitle = "More NTAs in Queens experience an increase in approval rates", caption = "Home Mortgage Disclosure Act Data, CFPB, 2013-2017")
```
...
cartogram(for_cartogram, "change",itermax=5)
